...
class HomeController extends Controller
{
    ...
    public function index(Column $column,Content $content,Files $files){
        ...
        $cases = $content->where('column_id',4)->where('recommend',1)->orderBy('updated_at','desc')->get();
        $newests = $content->where('column_id',9)->where('recommend',1)->orderBy('updated_at','desc')->get();
